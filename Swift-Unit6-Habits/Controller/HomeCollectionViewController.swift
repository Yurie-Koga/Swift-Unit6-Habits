    ...
    func createDataSource() -> DataSourceType {
        ...
        dataSource.supplementaryViewProvider = { (collectionView, kind, indexPath) in
            ...
            let view = collectionView.dequeueReusableSupplementaryView(
                ofKind: elementKind.viewKind, withReuseIdentifier: elementKind.reuseIdentifier, for: indexPath)
    ...
    func createLayout() -> UICollectionViewCompositionalLayout {
        let layout = UICollectionViewCompositionalLayout { (sectionIndex, environment) -> NSCollectionLayoutSection? in
            ...
            case .leaderboard:
                ...
                let groupDecorationSize = NSCollectionLayoutSize(widthDimension: .fractionalWidth(0.77),
                                                                 heightDimension: .fractionalWidth(0.77))
                let groupAnchor = NSCollectionLayoutAnchor(edges: .all, absoluteOffset: CGPoint(x: 0, y: 0))
                let groupDecoration = NSCollectionLayoutSupplementaryItem(
                    layoutSize: groupDecorationSize,
                    elementKind: SupplementaryView.leaderboardGroupBackground.viewKind,
